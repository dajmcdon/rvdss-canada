...
detections_data = get_detections_data(DASHBOARD_BASE_URL,headers,update_date)
positive_data = get_positive_data(DASHBOARD_BASE_URL,headers,update_date)

path1 = './data/season_2024_2025/respiratory_detections.csv'
path2 = './data/season_2024_2025/positive_tests.csv'

if os.path.exists(path1)==False:
    detections_data.to_csv(path1,index=True)
else:
    old_detection_data = pd.read_csv(path1).set_index(['epiweek', 'time_value', 'issue', 'geo_type', 'geo_value'])
    if detections_data.index.isin(old_detection_data.index).any() == False:
        old_detection_data= pd.concat([old_detection_data,detections_data],axis=0)
